...
**Экономия токенов:**
- Полный блок схем инструментов вшивается **один раз на тред**; на последующих ходах того же разговора — только однострочное напоминание.
...
- Это **мост на уровне промпта**, а не нативный function-calling — надёжен для типичного цикла с одним/параллельными вызовами, но не гарантирован для всех крайних случаев. Принимаются и блоки `<tool_call>{…}</tool_call>`, и fenced ```tool_call``` / ```json```.
- В tool-режиме ответ **буферизуется** (не токенами), чтобы надёжно ловить tool-вызовы.
