...
**Несколько аккаунтов и ротация.** Задай несколько токенов (`HYPERAGENT_SESSIONS=a,b,c`, `.txt`-файл или JSON-массив). Если сессия отдаёт 401/403 при создании треда, прокси автоматически **переключается на следующий аккаунт**.

**`.txt`-файл.** Укажи `SESSIONS_FILE` на файл (по токену на строку; можно комментарии `#`), либо просто положи `sessions.txt` в рабочую папку — подхватится автоматически.

> **Баланс показывается** — берётся вживую из `/api/settings/billing/*`: план, изначальный кредит, использовано и остаток. (Цифры — собственный pay-as-you-go кредит Hyperagent; прокси к ним ничего не добавляет.)
