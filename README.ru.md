...
- Это **мост на уровне промпта**, а не нативный function-calling — надёжен для типичного цикла с одним/параллельными вызовами, но не гарантирован для всех крайних случаев.
- В tool-режиме ответ **буферизуется** (не токенами), чтобы надёжно ловить tool-вызовы.
- Функции выполняются **на стороне клиента** (твой MCP), а не в Hyperagent.
- Для долгих циклов закрепляй разговор заголовком `X-Session-Id` (или `model@session`) — так переиспользование треда максимально стабильно.
